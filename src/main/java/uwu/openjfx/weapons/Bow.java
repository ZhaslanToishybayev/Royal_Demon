...
public abstract class Bow implements Weapon, AngleBehavior {
    ...
    protected static final double PLAYER_HIT_BOX_OFFSET_X = 3; // player's hitbox own offset from top left
    protected static final double PLAYER_HIT_BOX_OFFSET_Y = 15; // player's hitbox own offset from top left
    protected static final double PLAYER_HIT_BOX_WIDTH = 35; // width of player's hitbox from 3 to 38
    protected static final double PLAYER_HIT_BOX_HEIGHT = 40; // height of player's hitbox from 15 to 55
    @SuppressFBWarnings(value = "SS_SHOULD_BE_STATIC", justification = "Constants shared across Bow hierarchy.")
    protected static final double playerHitBoxOffsetX = PLAYER_HIT_BOX_OFFSET_X;
    @SuppressFBWarnings(value = "SS_SHOULD_BE_STATIC", justification = "Constants shared across Bow hierarchy.")
    protected static final double playerHitBoxOffsetY = PLAYER_HIT_BOX_OFFSET_Y;
    @SuppressFBWarnings(value = "SS_SHOULD_BE_STATIC", justification = "Constants shared across Bow hierarchy.")
    protected static final double playerHitBoxWidth = PLAYER_HIT_BOX_WIDTH;
    @SuppressFBWarnings(value = "SS_SHOULD_BE_STATIC", justification = "Constants shared across Bow hierarchy.")
    protected static final double playerHitBoxHeight = PLAYER_HIT_BOX_HEIGHT;
    ...
    @SuppressFBWarnings(value = "EI_EXPOSE_REP2", justification = "Weapon sprites are immutable and shared across instances.")
    ...
    public void prepAttack(Entity player) {
        ...
        b.getTransformComponent().setAnchoredPosition(
            new Point2D(
                (player.getX() + playerHitBoxOffsetX + (PLAYER_HIT_BOX_WIDTH / 2)) + bowOffsetX,
        ...
        if (!ultimateActivated) {
            ...
            rangedHitBox.setAnchoredPosition(
                (player.getX() + playerHitBoxOffsetX
                    + (player.getScaleX() > 0 ? PLAYER_HIT_BOX_WIDTH : 0)), // right-left side
    ...
    public void calculateAnglePlayerRelative(Entity player, double mouseCurrX, double mouseCurrY) {
        ...
        double adjacent = mouseCurrX
            - (player.getX() + PLAYER_HIT_BOX_OFFSET_X
            + (player.getScaleX() > 0 ? PLAYER_HIT_BOX_WIDTH : 0));
        double opposite = mouseCurrY
            - (player.getY() + PLAYER_HIT_BOX_OFFSET_Y
            + (PLAYER_HIT_BOX_HEIGHT / 2));
    ...
    @Override
    @SuppressFBWarnings(value = "EI_EXPOSE_REP", justification = "Sprites are immutable and shared between instances.")
